...
def test_follow_person_ros2_template_declares_every_adapter_it_uses():
    ...
    # The owned adapter, plus the arm adapter that owns ROS2ManualMove.
    assert workflow["metadata"]["required_adapters"] == [
        "blacknode-skills/follow@ros2",
        "blacknode-motion/arm@ros2",
    ]
    assert workflow["node_meta"]["subscribe"]["type"] == "ROS2SubscribeJointState"
    assert workflow["node_meta"]["publish"]["type"] == "ROS2JointController"
