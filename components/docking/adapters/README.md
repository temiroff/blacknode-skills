...
Transport adapters for the `docking` component of `blacknode-skills`.
...
Declare it in `blacknode-package.toml`:
    ...
    [components.docking.adapters.ros2]
    description = "ROS 2 adapter for docking."
    default = false
    capabilities = ["adapter.docking.ros2"]
    nodes = ["components/docking/adapters/ros2/nodes"]
